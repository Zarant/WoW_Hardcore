local _G = _G
local self_made_quad_achievement = CreateFrame("Frame")
_G.achievements.QuadMade = self_made_quad_achievement
local player = UnitName("Player")

-- General info
self_made_quad_achievement.name = "QuadMade"
self_made_quad_achievement.title = "Quad-Made"
self_made_quad_achievement.pts = 10
self_made_quad_achievement.class = "All"
self_made_quad_achievement.icon_path = "Interface\\Addons\\Hardcore\\Media\\icon_self_made.blp"
self_made_quad_achievement.description =
	"Complete the Hardcore challenge without at any point equipping an item that you or your quad have not crafted yourselves. Items your character has conjured (e.g. Firestones) are considered crafted. No items bought, dropped, or rewarded by quests are allowed to be equipped, fishing poles MAY be equipped. The items your character starts with are allowed to be equipped. Bags are equipped items."
local Combine = {
	35,
	36,
	38,
...
-- Registers
function self_made_quad_achievement:Register(fail_function_executor)
	self:RegisterEvent("PLAYER_EQUIPMENT_CHANGED")
	self:RegisterEvent("ITEM_UNLOCKED")
	self.fail_function_executor = fail_function_executor
end

function self_made_quad_achievement:Unregister()
...
self_made_quad_achievement:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "PLAYER_EQUIPMENT_CHANGED" then
		...
		if GetInventoryItemID("player", 0) ~= nil then
			...
			self_made_quad_achievement.fail_function_executor.Fail(self_made_quad_achievement.name)
		end
