local _G = _G
local self_made_penta_achievement = CreateFrame("Frame")
_G.achievements.PentaMade = self_made_penta_achievement
local player = UnitName("Player")

-- General info
self_made_penta_achievement.name = "PentaMade"
self_made_penta_achievement.title = "Penta-Made"
self_made_penta_achievement.pts = 10
self_made_penta_achievement.class = "All"
self_made_penta_achievement.icon_path = "Interface\\Addons\\Hardcore\\Media\\icon_self_made.blp"
self_made_penta_achievement.description =
	"Complete the Hardcore challenge without at any point equipping an item that you or your penta have not crafted yourselves. Items your character has conjured (e.g. Firestones) are considered crafted. No items bought, dropped, or rewarded by quests are allowed to be equipped, fishing poles MAY be equipped. The items your character starts with are allowed to be equipped. Bags are equipped items."
local Combine = {
	35,
	36,
	38,
...
-- Registers
function self_made_penta_achievement:Register(fail_function_executor)
	self:RegisterEvent("PLAYER_EQUIPMENT_CHANGED")
	self:RegisterEvent("ITEM_UNLOCKED")
	self.fail_function_executor = fail_function_executor
end

function self_made_penta_achievement:Unregister()
		...
		if region and region:GetObjectType() == "FontString" then
			...
			if text ~= nil then
				if string.match(text, player) then
					GameTooltip:Hide() --prevents a hung empty tooltip window
					return true
				elseif string.match(text, Hardcore_Character.team[1]) then
					GameTooltip:Hide() --prevents a hung empty tooltip window
					return true
				elseif string.match(text, Hardcore_Character.team[2]) then
					GameTooltip:Hide() --prevents a hung empty tooltip window
					return true
				elseif string.match(text, Hardcore_Character.team[3]) then
					GameTooltip:Hide() --prevents a hung empty tooltip window
					return true
				elseif string.match(text, Hardcore_Character.team[4]) then
					GameTooltip:Hide() --prevents a hung empty tooltip window
					return true
				end
			end
...
self_made_penta_achievement:SetScript("OnEvent", function(self, event, ...)
	...
	if event == "PLAYER_EQUIPMENT_CHANGED" then
		...
		if GetInventoryItemID("player", 0) ~= nil then
			...
			self_made_penta_achievement.fail_function_executor.Fail(self_made_penta_achievement.name)
		end
