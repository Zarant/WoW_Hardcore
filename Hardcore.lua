...
--Kevin START addition in Hardcore\Hardcore.lua
--  /run Hardcore:VersionCheck()
--  /run local nt,no,nm=GetNumGuildMembers() local ct=0 for i=1,no,1 do local n,r,ri,l,cl,_=GetGuildRosterInfo(i) if n then if Hardcore:GuildVersions(n)==nil then print(n) ct=ct+1 end end end print(ct,"guild members online without the Hardcore addon.")
function Hardcore:GuildVersions(player)
  return guild_versions[player]
end

function Hardcore:VersionCheck()
	local numTotal, numOnline, numOnlineAndMobile = GetNumGuildMembers();
  local numCounter = 0
  if display == "AddonStatus" and Hardcore_SubTitle:GetText() == STRING_ADDON_STATUS_SUBTITLE then
    for i = 1, numOnline, 1 do
      local name, rankName, rankIndex, level, classDisplayName, zone, publicNote, officerNote, isOnline, status,
        class, achievementPoints, achievementRank, isMobile, canSoR, repStanding, GUID = GetGuildRosterInfo(i)
      if name then
        if guild_versions[name] == nil then
          numCounter = numCounter + 1
          print("Name:", name, "|", "Level:", level)
          --add logic here when to guild kick people who don't have the addon installed and allow for any desired exceptions
          --GuildUninvite(name);
        end
      end 
    end
    print("In your guild, there are", numCounter, "guild members online who are not using the Hardcore addon.")
  else
    print("Open the Accountability page in the Hardcore addon and wait for it to fully load.")
  end
end

--  /run Hardcore:ExplainVerificationString()
--  /run Hardcore:ExplainFakeVerificationString()
function Hardcore:ExplainVerificationString()
  local strInput = Hardcore:GenerateVerificationString()
  local strOutput = ""
  local tableMatches = {}
  print(strInput)
  for strMatch in string.gmatch(string.gsub(strInput, "[_]+", "f126f"), "%d*") do
      table.insert(tableMatches, tonumber(strMatch))
  end
  for index, strEntry in ipairs(tableMatches) do
    strOutput = strOutput..string.char(strEntry)
  end
  print(strOutput)
end

function Hardcore:GenerateFakeVerificationString()
	local _, class, _, race, _, name = GetPlayerInfoByGUID(UnitGUID("player"))
	local realm = GetRealmName()
	local level = 60
  local null = ""
	...
	local tradePartners = Hardcore_join(Hardcore_Character.trade_partners, ",")
	local baseVerificationData = {Hardcore_Character.guid, realm, race, class, name, level,
									Hardcore_Character.time_played, Hardcore_Character.time_played-160,
									0, tradePartners}
	local baseVerificationString = Hardcore_join(Hardcore_map(baseVerificationData, Hardcore_stringOrNumberToUnicode),
		ATTRIBUTE_SEPARATOR)
	local bubbleHearthIncidentsVerificationString = Hardcore_tableToUnicode(Hardcore_Character.bubble_hearth_incidents)
	local playedtimeGapsVerificationString = Hardcore_tableToUnicode(Hardcore_Character.played_time_gap_warnings)
	return Hardcore_join({baseVerificationString, null, null}, ATTRIBUTE_SEPARATOR)
end

function Hardcore:ExplainFakeVerificationString()
  local strInput = Hardcore:GenerateFakeVerificationString()
  local strOutput = ""
  local tableMatches = {}
  print(strInput)
  for strMatch in string.gmatch(string.gsub(strInput, "[_]+", "f126f"), "%d*") do
